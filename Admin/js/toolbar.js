...
var _ttTop		= 3;	//toolbar target is top window
	...
	else
	{
		if(Item && Item.Url)
		{
			if(Item.Target == _ttSelf)
			{
				...
				if(window.event.shiftKey) frm = window.open('about:blank');
				
				if(!frm) frm = window;

				frm.location = Item.Url;
			}
			else if (Item.Target == _ttBlank)
			{
				window.open(Item.Url);
			}
			else eval('window.top.location = "' + Item.Url + '"');
		}
	}
}
