	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="normal">
		<tr class="adminheader">
			<td width="33%">
				...
				<span class="module_title">交费记录</span> &nbsp;&nbsp; <a href="{CURRENT_MODULE}">{LANG_showall}</a> 
			</td>
			<td>
...
<form action="{CURRENT_ACT}" method="post" name="frmInfo">
	<table width="100%"  border="0" cellspacing="0" cellpadding="1" class="adminlist">
		<tr class="list_title">
			...
			<th id="orderby" orderfield="Money" >金额</th>
			<th id="orderby" orderfield="TotalDays" >晚托天数</th>
			<th id="orderby" orderfield="StartTime" >开始时间</th>
			<th id="orderby" orderfield="EndTime" >结束时间</th>
			<th id="orderby" orderfield="AddTime" >交费时间</th>
		...
		<tr align="center" class="list_content" onClick="select_thisrow(this);" onDblClick="editItem({RowList.ID})" onmouseover="highlightrow2(this, true);" onmouseout="highlightrow2(this, false);">
			...
			<td>{RowList.StuNO}</td>
			<td>{RowList.Money}</td>
			<td>{RowList.TotalDays}</td>
			<td>{RowList.StartTime}</td>
			<td>{RowList.EndTime}</td>
