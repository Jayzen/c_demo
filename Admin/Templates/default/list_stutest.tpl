	...
	<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="normal">
		<tr class="adminheader">
			<td width="33%">
				...
				<span class="module_title">学生测试考试表</span> &nbsp;&nbsp; <a href="{CURRENT_MODULE}">{LANG_showall}</a> 
			</td>
			<td>
...
<form action="{CURRENT_ACT}" method="post" name="frmInfo">
	<table width="100%"  border="0" cellspacing="0" cellpadding="1" class="adminlist">
		<tr class="list_title">
			...
			<th id="orderby" orderfield="TestID" >测试名称</th>
			<th id="orderby" orderfield="TestID" >测试时间</th>
			<th id="orderby" orderfield="Score" >分数</th>
			<th id="orderby" orderfield="Remark" >评论</th>
			<th id="orderby" orderfield="PaperImage" >试卷图片</th>
		...
		<tr align="center" class="list_content" onClick="select_thisrow(this);" onDblClick="editItem({RowList.ID})" onmouseover="highlightrow2(this, true);" onmouseout="highlightrow2(this, false);">
			...
			<td>{RowList.StuNO}</td>
			<td>{RowList.TestID}</td>
			<td>{RowList.TestTime}</td>
			<td>{RowList.Score}</td>
			<td>{RowList.Remark}</td>
			<td>{RowList.PaperImage}</td>
